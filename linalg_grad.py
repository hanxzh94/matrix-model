...
def _MatrixInverseGrad(op, grad):
  """Gradient for MatrixInverse."""
  ainv = op.outputs[0]
  adj = op.get_attr("adjoint")
  if adj:
    return -math_ops.matmul(
        ainv, math_ops.matmul(grad, ainv, adjoint_a=True))
  else:
    return -math_ops.matmul(
        ainv, math_ops.matmul(grad, ainv, adjoint_b=True), adjoint_a=True)
...
def _SelfAdjointEigV2Grad(op, grad_e, grad_v):
  ...
  with ops.control_dependencies([grad_e, grad_v]):
    if compute_v:
      ...
      diff = array_ops.expand_dims(e, -2) - array_ops.expand_dims(e, -1)
      r = math_ops.conj(diff) * math_ops.cast(math_ops.reciprocal(math_ops.real(diff * math_ops.conj(diff))), diff.dtype.base_dtype)
